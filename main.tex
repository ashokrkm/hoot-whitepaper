...
Broadcasted video are also continuously archived, and will be stored in decentralized file system IPFS \footnote{https://ipfs.io/}
...
\item[-]Modular architecture allows building Tor/VPN modules inorder
  to enable uncensorable live-streaming to promote free speech
\end{itemize}
...
%inlcude faster input imports all commands http://i.imgur.com/YsPQMeD.png include{filename} gets you the speed bonus, but it also can't be nested, can't appear in the preamble, and forces page breaks around the included text by using  \ clearpage before and after the content of the file.. 
...
\sout{For micro-payments, artists can safely accept the hootcoin payment immediately.  The size of the payment is too small for the effort to steal it. Micro-payments are almost always for intellectual property, where there's no physical loss to the merchant. }
