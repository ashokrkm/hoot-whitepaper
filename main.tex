...
\section{Blockstack, Toshi, Status and the beginnings of decentralized
  web}
...
\section{Auction and finding best price for excess compute}
To bootstrap the network, Hoot network will continuously run Vickrey auctions to find the best service to run on the miners computer that has excess capacity. (A \emph{Vickrey auction} is one in which the winner pays the second-highest price, not the price they themselves bid).
...
\emph{including} amendments to the auction amendment procedure itself.
Upgrades to Hoot auction protocol are staged through a testing environment to allow stake-holders to recall potentially inferior amendments. 
