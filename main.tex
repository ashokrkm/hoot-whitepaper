...
\section{Security}
Signup, interaction and archived static content are end-to-end HTTPS  SSL encrypted
to ensure strong security. The live connection is encrypted using AES_256_CBC, with HMAC-SHA1 for message authentication and
DHE_RSA as the key exchange mechanism. Every RTMP player connection is authenticated with an RTMPE/ RTMPS
connection. The Hoot iOS and android player endpoints support RTMPE, RTMPS
An authorization key is needed to view a private Hoot video stream. HLS streams are HTTPS encrypted.
